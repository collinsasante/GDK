...
const SingleCourseFour = (props) => {
    const { itemClass, courseID, courseImg, courseTitle, courseName, courseAuthor, courseAuthorImg, courseLesson, courseEnrolled, coursePrice, courseReview } = props;
    ...
    const handleEnrollClick = (e) => {
        ...
        } else if (isEnrolled(user.id, courseID)) {
            navigate('/my-lessons');
        } else {
	...
	return(
        <div className={itemClass ? itemClass : 'inner-course'}>
            ...
            <div className="case-content">
                ...
                <button
                    ...
                    style={{
                        ...
                        marginTop: '15px',
                        padding: '12px',
                        backgroundColor: enrolled ? '#4CAF50' : '#2196F3',
                        color: 'white',
                        border: 'none',
                        borderRadius: '4px',
                        fontSize: '16px',
                    ...
                    onMouseOver={(e) => e.target.style.backgroundColor = enrolled ? '#45a049' : '#1976D2'}
                    onMouseOut={(e) => e.target.style.backgroundColor = enrolled ? '#4CAF50' : '#2196F3'}
                >
                    {enrolled ? 'View Lesson' : 'Enroll Now'}
                </button>
            </div>
        </div>
	)
